...
import { analyzeCommitmentGraph } from '@/services/GuardianGraphService';
    ...
    try {
        ...
        if (commitments.length === 0) {
            return NextResponse.json(
                { status: 'invalid_input', reason: 'Nenhum compromisso enviado para correlação.' },
                { status: 400 },
            );
        }

        const result = await analyzeCommitmentGraph(commitments);
        return NextResponse.json(result, { status: 200 });
    } catch {
        return NextResponse.json(
            { status: 'unavailable', reason: 'Erro inesperado no Graph Engine.' },
            { status: 500 },
        );
    }
}
