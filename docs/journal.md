...
- **Governança:** Conclusão da SAFE-005 (Governança de Prazos). Bloqueio implementado na interface (HTML5 `min` attribute estrito) com retaguarda no backend (validação de serviço) para impedir rigorosamente que compromissos sejam alocados em datas passadas.
- **Auditoria e Histórico:** Conclusão da CORE-027 (Edição Completa do Compromisso). Adicionada capacidade de editar título, dono, projeto e data. O sistema detecta exatamente o que mudou e lança um `AuditEvent` rastreável (tipo EDIT ou RENEGOTIATION) garantindo plena transparência na evolução dos combinados.
- **Bug Registrado (FIX-001):** Identificado crash no formulário `CommitmentForm` ao apagar a data/deixar campos vazios, exigindo tratamento para não quebrar o React runtime. Inclusão no backlog para correção futura. Em seguida, iniciada a CORE-026.
